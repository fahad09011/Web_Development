...
// coursenyear the varibles that stores the input(course year) 
let date_of_course = document.getElementById("dob");
// datecheck is a function that checks/validate  the age of person as 16 or older than 16
let courseYearCheck= (enterdate)=>{
    // Date () is predefined function for current date
    let today = new Date();
    // if its true then return otherwise fale
    if (enterdate < today) {
...
form.addEventListener("submit", (f)=>{
    ...
    // setCustomValidity() is a predefined function to show custom error message while subitting form
    date_of_course.setCustomValidity("");
    let enter_course_date = new Date(date_of_course.value)
    if (!courseYearCheck(enter_course_date)) {
        // preventDefault() :predefined function that prevents the form from submission if the course date is not correct
        f.preventDefault();
        date_of_course.setCustomValidity("Course date must be before today");
        // reportValidity() show the error/validation message immidiately when needed
        date_of_course.reportValidity();
    }
...
date_of_course.addEventListener("input", () => {
    let entercoursedate = new Date(date_of_course.value);
    if (courseYearCheck(entercoursedate)) {
        date_of_course.setCustomValidity(""); // Clear error when valid
    }
});
